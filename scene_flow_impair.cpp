...
void PD_flow_opencv::initializeCUDA(unsigned int rows_, unsigned int cols_)
{
    width = cols_;
    height = rows_;
...
{
    intensity1 = cv::imread("i1.png", CV_LOAD_IMAGE_GRAYSCALE);
    initializeCUDA(intensity1.rows, intensity1.cols);
}
...
{
    cv::Mat
        i1 = cv::imread("i1.png", CV_LOAD_IMAGE_GRAYSCALE),
        i2 = cv::imread("i2.png", CV_LOAD_IMAGE_GRAYSCALE),
        d1 = cv::imread("z1.png", -1),
        d2 = cv::imread("z1.png", -1);
    return loadRGBDFrames(i1, d1, i2, d2);
}

bool PD_flow_opencv::loadRGBDFrames(const cv::Mat &i1, const cv::Mat &d1,
    const cv::Mat &i2, const cv::Mat &d2)
{
    ...
    depth1.convertTo(depth_float, CV_32FC1);
    for (unsigned int v = 0; v < height; v++)
        for (unsigned int u = 0; u < width; u++)
    ...
    depth2.convertTo(depth_float, CV_32FC1);
    for (unsigned int v = 0; v < height; v++)
        for (unsigned int u = 0; u < width; u++)
            Z[v + u*height] = depth_float.at<float>(v, u);
